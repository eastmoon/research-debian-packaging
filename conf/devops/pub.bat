...
:args
    ...
    if "%KEY%"=="--content" (set CONTENT_DIRECTORY=%VALUE%)
    if "%KEY%"=="-c" (set CONTENT_DIRECTORY=%VALUE%)
    goto end
...
:help
    ...
    echo      --content, -c     Content directory ( Default use %RC_PUBLISH_DEFAULT_CONTENT_DIR:/=\% )
